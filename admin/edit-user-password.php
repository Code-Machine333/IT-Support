...
if (strlen($_SESSION['adid'] == 0)) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $query = mysqli_query($con, "select ID from tbluser where ID='$sid'");
        $row = mysqli_fetch_array($query);
        if ($row > 0) {
            $ret = mysqli_query($con, "update tbluser set Password='$newpassword' where ID='$sid'");
                                    ...
                                    <div class="col-12">
                                        <div class="p-20">
                                            ...
                                            <form class="form-horizontal" role="form" method="post" name="changepassword" onsubmit="return checkpass();">
                                                <?php
                                                $sid = substr(base64_decode($_GET['udid']), 0, -5);
                                                $ret = mysqli_query($con, "select ID, FullName, Email, RegDate, active, admin from tbluser where ID='$sid'");
                                                list($id, $FullName, $Email, $RegDate, $active, $admin) = @mysqli_fetch_row($ret);
                                                ...
                                                <div class="form-group row">
                                                    ...
                                                    <div class="col-12">
                                                        <p style="text-align: center;">
                                                            <button type="submit" name="submit" class="btn-custom-primary">Change</button>
                                                        </p>
                                                    </div>
